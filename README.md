...
- **MCP server** — built-in MCP (Model Context Protocol) endpoint at `/mcp` using Streamable HTTP transport; exposes the wiki as tools for AI assistants (list/read/create/edit/delete pages, search, image upload via base64 or URL, history, link analysis, MediaWiki import). Connect from Claude using a remote MCP custom connector
